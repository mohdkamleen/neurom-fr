  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
        <div style={{ width: "60px", height: "60px", background: "lightgray", borderRadius: "50px", display: "grid", placeItems: "center", fontSize: "20px", margin: "10px 0px" }}>{user?.username.length > 0 && (user.username[0] + (user.username.split(" ")[1] ? user.username.split(" ")[1][0] : "")).toUpperCase()}</div>
        {user?.username &&
          <div style={{ fontSize: "18px", marginTop: "18px" }}>
            <b>{user?.username.toUpperCase()}</b> {user?.verified && <VerifiedOutlined />} <br />
            <p> {user.phone} </p>
          </div>}
      </div>
      <h3>{user?.email}</h3>
      <AimOutlined /> {team?.address ? team.address : <i>team location</i>}
      <br /><br /><br />
      ...
      <big onClick={() => navigate("/manageteam")} style={{ width: "100%", cursor: "pointer", padding: "7px 0", maxWidth: "500px", display: "flex", justifyContent: "space-between", alignItems: "center" }}> Manage Team <RightOutlined style={{ fontSize: "12px" }} /> </big>
